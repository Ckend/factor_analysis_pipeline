...
class CSVLoder:
    ...
    def get_factor_data(
        ...
        fields: List[str] = list({"Date", "Ticker"}.union(fields))
        ...
        df["Date"] = pd.to_datetime(df["Date"])
        if codes:
            df: pd.DataFrame = df[df["Ticker"].isin(codes)]
        return df
    ...
    def get_stock_price(
        ...
        df["columns"] = df.index
        df = df.melt(id_vars=["Ticker", "columns"], var_name="Date", value_name="Value")
        df.sort_values("Date", inplace=True)
