...
def prepare_params(loader) -> Dict:
    ...
    factor_data: pd.DataFrame = loader.get_factor_data(
        start_dt=start_dt, end_dt=end_dt, fields=factor_names
    )
    factor_data.dropna(inplace=True)
    codes: List[str] = factor_data["Ticker"].unique().tolist()
    price: pd.DataFrame = loader.get_stock_price(
        codes, start_dt=start_dt, end_dt=end_dt, fields=st.session_state["price_type"]
    )
    price[st.session_state["price_type"]] = price[st.session_state["price_type"]].astype(float)
    pricing: pd.DataFrame = pd.pivot_table(
        price, index="Date", columns="Ticker", values=st.session_state["price_type"]
    )
...
def load_analyzer(factor_name: str, params: Dict) -> Tuple[str, FactorAnalyzer]:
    ...
    factor_ser = factor_ser[factor_name]
    factor_ser = factor_ser.fillna(0)
