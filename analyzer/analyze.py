...
class FactorAnalyzer(object):
    ...
    def __gen_clean_factor_and_forward_returns(self):
        ...
        stocks = factor_data.index.get_level_values(1).drop_duplicates().tolist()
    ...
    def _factor_quantile(self):
        ...
        if not data.empty:
            return np.max(data.factor_quantile)
        else:
    ...
    def plot_quantile_average_cumulative_return(self, periods_before=5, periods_after=10,
        ...
        average_cumulative_return_by_q = average_cumulative_return_by_q.sort_index(axis=1)
