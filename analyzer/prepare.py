...
def compute_forward_returns(
    factor: pd.Series, prices: pd.DataFrame, periods: Tuple = (1, 5, 10)
) -> pd.DataFrame:
    """
    ...
    factor_dateindex: pd.DatetimeIndex = factor.index.levels[0]
    factor_dateindex: pd.DatetimeIndex = factor_dateindex.intersection(prices.index)
    ...
    prices: pd.DataFrame = prices.filter(items=factor.index.levels[1])
...
def rate_of_return(period_ret):
    ...
    return period_ret.add(1).pow(1.0 / period).sub(1)
